...
export function HashtagsCard() {
  ...
  return (
    <GlassCard className="border p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 sm:mb-6">
        <div>
          <h3 className="text-xl sm:text-2xl font-extrabold text-foreground flex items-center gap-2 transition-colors duration-500">
            ...
            <Hash className="w-4 h-4 sm:w-5 sm:h-5 text-purple-600 dark:text-neonCyan" />
          </h3>
          <p className="text-[11px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 transition-colors duration-500">Smart, non-generic hashtags grouped for maximum reach.</p>
        </div>
      ...
      <div className="space-y-4 sm:space-y-5">
        {analysisResult.hashtags.map((group) => (
          <div key={group.category}>
            <h4 className="text-[10px] sm:text-xs text-muted-foreground mb-2 uppercase tracking-widest font-semibold flex items-center gap-1.5 transition-colors duration-500">
              <span className="w-1.5 h-1.5 rounded-full bg-purple-600 dark:bg-neonCyan shrink-0"></span>
              ...
              {group.tags.map(tag => {
                ...
                return (
                  <Badge
                    ...
                    variant="outline"
                    className={cn(
                      "cursor-pointer text-[11px] sm:text-xs py-1 sm:py-1.5 px-2.5 sm:px-3 transition-all hover:scale-105 rounded-xl border font-sans font-medium",
                      isActive 
