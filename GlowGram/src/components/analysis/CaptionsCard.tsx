...
export function CaptionsCard() {
  ...
  return (
    <GlassCard className="flex flex-col h-full border">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-2xl font-extrabold text-foreground flex items-center gap-2 transition-colors duration-500">
            ...
            <Sparkles className="w-5 h-5 text-purple-600 dark:text-neonPurple" />
          </h3>
          <p className="text-xs text-muted-foreground mt-1 transition-colors duration-500">Contextual AI captions tailored to your uploaded media.</p>
        </div>
      </div>
      
      <Tabs defaultValue={defaultTab} className="w-full flex-1 flex flex-col">
        <TabsList className="w-full justify-start overflow-x-auto bg-purple-100/60 dark:bg-black/40 border border-purple-200/50 dark:border-white/10 p-1.5 rounded-xl h-auto flex-wrap gap-1 transition-colors duration-500">
          {availableTones.map(tone => (
            <TabsTrigger 
              key={tone} 
              value={tone}
              className="rounded-lg text-sm px-3.5 py-1.5 data-[state=active]:bg-purple-200/80 data-[state=active]:text-purple-950 data-[state=active]:border-purple-300 dark:data-[state=active]:bg-neonPurple/20 dark:data-[state=active]:text-white dark:data-[state=active]:border-neonPurple/40 border border-transparent transition-all font-medium"
            >
              {tone}
            </TabsTrigger>
          ))}
          ...
          return (
            <TabsContent key={tone} value={tone} className="flex-1 mt-4 flex flex-col gap-4">
              <Textarea 
                ...
                className="flex-1 min-h-[160px] bg-white/90 dark:bg-black/50 border-purple-200/60 dark:border-white/10 text-foreground text-base leading-relaxed resize-none focus-visible:ring-purple-500 rounded-xl p-4 transition-colors duration-500 font-sans"
              />
              <div className="flex justify-between items-center">
                <span className="text-xs text-muted-foreground font-mono transition-colors duration-500">{caption.text.length} characters</span>
                <div className="flex gap-2">
                  <Button 
                    variant="outline" 
                    size="sm" 
                    className="border-purple-200 dark:border-white/20 text-foreground bg-purple-50/50 dark:bg-transparent hover:bg-purple-100 dark:hover:bg-white/10 rounded-lg text-xs transition-colors duration-500"
