...
export function UploadZone() {
  ...
  return (
    <div {...getRootProps()} className={cn("cursor-pointer transition-all duration-300", isDragActive ? "scale-[1.02]" : "")}>
      <GlassCard 
        className={cn(
          "border-2 border-dashed transition-all duration-300 ease-in-out text-center py-16 rounded-3xl",
          isDragActive 
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-bold text-foreground mb-1.5 transition-colors duration-500">
          ...
          <div className="mt-3 flex items-center gap-2 text-xs font-medium text-muted-foreground bg-purple-100/60 dark:bg-black/40 px-4 py-2 rounded-full border border-purple-200/40 dark:border-white/10 transition-colors duration-500">
            <span className="flex items-center gap-1.5"><ImageIcon className="w-3.5 h-3.5 text-pink-500 dark:text-neonPink" /> JPEG, PNG, WEBP</span>
            <span className="opacity-40">|</span>
            <span className="flex items-center gap-1.5"><Film className="w-3.5 h-3.5 text-cyan-500 dark:text-neonCyan" /> MP4, MOV</span>
          </div>
        </div>
      </GlassCard>
    </div>
